...
//        dd($nextMonday,$prevMonday);
        ...
        foreach ($managers as $manager) {
            if ($manager->hasRole('manager')) {
                ...
                foreach ($manager_days as $manager_day) {
                    $leads = Lead::whereDate("created_at", Carbon::createFromDate($manager_day["date"])->toDateString())->where([["manager_id", '=', $manager->id], ["entered", "!=", null]])->get();
                    $manager_days[$counter]["repairs"] = count($leads);
                    $counter++;
                }
...
//                        dd($manager_days,$dt,$manager_gsm,$mg,$found_key);
                ...
                $isGsmTotallyPayed=true;


                foreach ($manager_days as $manager_day){
                    if($manager_day['gsm_is_payed']==false&&$manager_day['gsm']!=0){
                        $isGsmTotallyPayed=false;
                    }
                }
                array_push($managers_gsm, [$manager, $manager_days, $total_gsm,$isGsmTotallyPayed]);
...
//        dd(Lead::whereDate("created_at",Carbon::createFromDate($day["date"])->toDateString())->where(["manager_id"=>$manager->id])->get());
...
//        dd($nextMonday,$prevMonday);
        ...
        foreach ($managers as $manager) {
            if ($manager->hasRole('master')) {
                ...
                foreach ($manager_days as $manager_day) {
                    $leads = Repair::whereDate("repair_date", Carbon::createFromDate($manager_day["date"])->toDateString())->where(["master_id" => $manager->id])->get();
                    $manager_days[$counter]["repairs"] = count($leads);
                    $counter++;
                }
...
//        dd($managers_gsm);
    ...
    public function payAllGsm(Request $request)
    {
        $role = $request->get('role');
        $manager = User::where(["id" => $request->get('manager')])->first();
        $start_date = $request->get('start_date');
        $end_date = $request->get('end_date');

        $gsms = Gsm::whereBetween('created_at', [$start_date, $end_date])->where(['user_id' => $manager->id])->get();
